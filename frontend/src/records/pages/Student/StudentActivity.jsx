            ...
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white rounded-lg overflow-hidden border">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="py-2 px-4 text-center align-middle whitespace-nowrap w-20">Photo</th>
                    <th className="py-2 px-4 text-center align-middle whitespace-nowrap">Relationship</th>
                    <th className="py-2 px-4 text-center align-middle whitespace-nowrap">Name</th>
                    <th className="py-2 px-4 text-center align-middle whitespace-nowrap">Age</th>
                    <th className="py-2 px-4 text-center align-middle whitespace-nowrap">Occupation</th>
                    <th className="py-2 px-4 text-center align-middle whitespace-nowrap">Income</th>
                    <th className="py-2 px-4 text-center align-middle whitespace-nowrap">Phone</th>
                    <th className="py-2 px-4 text-center align-middle whitespace-nowrap">Email</th>
                  </tr>
                </thead>
                  ...
                  {student.studentUser.relationDetails.map((relation, index) => {
                    ...
                    const photoUrl = photoPath && photoPath !== '/uploads/default.jpg'
                      ? (photoPath.startsWith('http') || photoPath.startsWith('data:')
                          ? photoPath
                          : `${(config?.backendUrl || config?.API_BASE_URL || 'http://localhost:4000').replace(/\/api$/, '')}${photoPath.startsWith('/') ? '' : '/'}${photoPath}`)
                      : null;
                    return (
                      <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        <td className="py-2 px-4 text-center align-middle">{relation.relationship || '-'}</td>
                        <td className="py-2 px-4 text-center align-middle">{relation.relation_name || '-'}</td>
                        <td className="py-2 px-4 text-center align-middle">{relation.relation_age || '-'}</td>
                        <td className="py-2 px-4 text-center align-middle">{relation.relation_occupation || '-'}</td>
                        <td className="py-2 px-4 text-center align-middle">₹{relation.relation_income || '0'}</td>
                        <td className="py-2 px-4 text-center align-middle">{relation.relation_phone || '-'}</td>
                        <td className="py-2 px-4 text-center align-middle">{relation.relation_email || '-'}</td>
                      </tr>
                    );
                  })}
                </tbody>
