...
const StaffPlacementFeedback = () => {
  ...
  const downloadAllPDF = async () => {
    ...
    try {
      ...
      const response = await api.get('/placement/feedback/bulk-pdf', {
        params: activeFilters,
        responseType: 'blob',
        headers: {
                        ...
                        <tr key={feedback.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 text-sm font-medium">
                            <div className="flex gap-2">
                              ...
                              <button
                                onClick={() => downloadSinglePDF(feedback.id, feedback.student_name, feedback.company_name)}
                                disabled={downloadingIds.has(feedback.id)}
                                className="inline-flex items-center gap-1 px-3 py-1 bg-green-600 text-white text-xs rounded hover:bg-green-700 transition-colors disabled:bg-green-400"
                              >
                                <Download className="h-3 w-3" />
                                {downloadingIds.has(feedback.id) ? 'Downloading...' : 'PDF'}
                              </button>
                            </div>
                          </td>
                        </tr>
...
export default StaffPlacementFeedback;
