...
export const testSkillRackSetup = async (req, res) => {
  try {
    ...
    console.log("📊 Model loading status:", tests.modelsLoaded);
    console.log("🔧 Model methods status:", tests.modelMethods);

    // Test database connection
    let dbTest = { success: false, error: null, count: 0 };
    try {
      console.log("🔍 Testing database connection with SkillRack.count()...");
      const count = await SkillRack.count();
      dbTest = { success: true, error: null, count };
      console.log("✅ Database test successful, count:", count);
    } catch (err) {
    ...
    try {
      console.log("👤 Testing student lookup for UserId:", testUserId);
      ...
      console.error("❌ Student test failed:", err);
