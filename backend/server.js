...
import staffEducationRoutes from './routes/staff/educationRoutes.js';
...
app.use(cors({
  origin: (origin, callback) => {
    const allowedOrigins = (process.env.FRONTEND_URL || 'http://localhost:5173,http://localhost:5174').split(',').map(o => o.trim());
...
// app.use('/api', authRoutes); // DISABLED: Using MySQL auth from app.js to fix 500 error
...
app.use('/api/staff/education', staffEducationRoutes);
// app.use('/api/auth', authRoutes); // DISABLED: Using MySQL auth from app.js to fix 500 error
